// this is the single expense card component used in the expense tab 
...
const SingleExpenseCard = React.memo(({expenseId, title, amount, date, description, category}) => {

    // an object that stores category to its iconify icon name (to fetch it from iconify)
    const categoryToIcon = {
        Education:"mdi:education-outline",
        Groceries:"maki:grocery",
        Health:"solar:health-bold",
        Subscriptions : "material-symbols:subscriptions-sharp",
        Takeaways:"ri:takeaway-fill",
        "Bank Transfer":"bxs:bank",
        Clothing:"game-icons:clothes",
        Electronic:"healthicons:electricity",
        Travelling:"material-symbols:travel",
        Installment:"vaadin:money-withdraw",
        Other:"material-symbols-light:other-admission-rounded"
    }

    // this func gets expenseId in parameter and deleted that particular expense card frm db
    const deleteExpense = async (expenseId) => {
        //    /api/income/delete-expense/:expenseId
        try{
            const response = await makeAuthenticatedPOSTRequest("/api/expense/delete-expense/" + expenseId); 
            if(response.err){
                alert(response.err);
                return;
            }
            else{
                alert(`${title} expense deleted`);
    ...
    return(
        <div className='single-income-card-div w-full border border-white bg-white flex items-center py-3 px-3 rounded-2xl space-x-4 hover:border hover:border-red-700 shadow-lg'>
            ...
            <div className='w-4/5 space-y-2'>
                ...
                <div className=' w-full flex items-center space-x-2 '>
                    <Icon icon="icon-park-outline:dot" fontSize={23} className='text-red-600'/>
